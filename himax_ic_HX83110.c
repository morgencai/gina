...
int himax_read_ic_trigger_type(struct i2c_client *client)
{
	...
	himax_burst_enable(client, 0);
	tmp_data[3] = 0x08; tmp_data[2] = 0x00; tmp_data[1] = 0x00; tmp_data[0] = 0x8C;

	result = i2c_himax_write(client, 0x00 ,tmp_data, 4, HIMAX_I2C_RETRY_TIMES);
	if (result < 0)
	{
			E("%s: i2c access fail!\n", __func__);
			return result;
	...
	tmp_data[0] = 0x00;
	result = i2c_himax_write(client, 0x0C ,tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	if (result < 0)
	{
			E("%s: i2c access fail!\n", __func__);
			return result;
	}
	result = i2c_himax_read(client, 0x08 ,read_data, 1, HIMAX_I2C_RETRY_TIMES);
	if (result < 0)
	{
		E("%s: i2c access fail!\n", __func__);
		return result;
